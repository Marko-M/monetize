...
class Monetize_Clicks_List_Table extends WP_List_Table {
    ...
    function __construct(){
        parent::__construct( array(
            'singular'  => __('click', 'monetize'),
            'plural'    => __('clicks', 'monetize'),
            'ajax'      => false
        ) );
    ...
    public function display_clicks_trends() {
        $this->filter_html();

?>
    <div id="monetize-clicks-trends-chart"
         class="monetize-charts"
         data-monetize-clicks-trends='<?php echo json_encode($this->trends) ?>'></div>
<?php
    }
        ...
        switch($column_name){
            case 'impression_ip':
                if(current_user_can(Monetize::admin_cap)) {
                    $actions = array(
                        'delete'    => sprintf('<a href="?page=%s&action=%s&click_id=%s">'.__('Delete', 'monetize').'</a>',$_REQUEST['page'],'delete',$item['click_id']),
                    );
    ...
    function process_bulk_action() {
        ...
        $ids = isset($_REQUEST['click_id']) ? $_REQUEST['click_id'] : array();

        if($this->current_action() === 'delete') {
            $monetize->delete_clicks($ids);
        }
    }

    public function prepare_clicks_trends() {
        global $monetize;

        $this->trends = $monetize->get_clicks_trends (
    ...
    function prepare_items() {
        ...
        $orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'click_created';
        ...
        $total_items = $monetize->get_clicks_count(
            $this->start_timestamp,
            $this->end_timestamp,
            $this->user_id
        );

        $this->items = $monetize->get_clicks(
            $this->start_timestamp,
            $this->end_timestamp,
            $orderby,
            $order,
